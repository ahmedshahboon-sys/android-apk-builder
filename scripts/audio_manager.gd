...
func update_engine(speed_ratio: float, accelerating: bool) -> void:
    if engine_player == null:
        return
    var r := clamp(abs(speed_ratio), 0.0, 1.0)
...
func _make_tone(freq: float, seconds: float, gain: float, looped: bool) -> AudioStreamWAV:
    ...
    for i in range(count):
        ...
        var env := 1.0
        if not looped:
            var attack := min(1.0, t / 0.015)
            var release := min(1.0, (seconds - t) / 0.05)
            env = min(attack, release)
        var harmonic := sin(TAU * freq * t) * 0.75 + sin(TAU * freq * 2.0 * t) * 0.18
        var v := int(clamp(128.0 + harmonic * gain * env * 120.0, 0.0, 255.0))
        bytes[i] = v
...
func _make_noise(seconds: float, gain: float, looped: bool) -> AudioStreamWAV:
    ...
    for i in range(count):
        ...
        smooth = lerp(smooth, raw, 0.08)
        var v := int(clamp(128.0 + smooth * gain * 120.0, 0.0, 255.0))
        bytes[i] = v
    var wav := AudioStreamWAV.new()
    wav.format = AudioStreamWAV.FORMAT_8_BITS
    wav.mix_rate = rate
...
func _make_chime(freqs: Array, note_seconds: float, gain: float) -> AudioStreamWAV:
    var rate := 22050
    var total_seconds := note_seconds * float(freqs.size())
    var count := int(total_seconds * rate)
    var bytes := PackedByteArray()
    bytes.resize(count)
    for i in range(count):
        var t := float(i) / float(rate)
        var note_index := min(int(t / note_seconds), freqs.size() - 1)
        var local_t := fmod(t, note_seconds)
        var freq := float(freqs[note_index])
        var env := min(1.0, local_t / 0.01) * min(1.0, (note_seconds - local_t) / 0.04)
        var wave := sin(TAU * freq * local_t)
        bytes[i] = int(clamp(128.0 + wave * gain * env * 120.0, 0.0, 255.0))
    var wav := AudioStreamWAV.new()
    wav.format = AudioStreamWAV.FORMAT_8_BITS
    wav.mix_rate = rate
    wav.stereo = false
