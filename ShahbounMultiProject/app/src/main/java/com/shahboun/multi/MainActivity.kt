...
class MainActivity : AppCompatActivity() {
    ...
    private fun manage(c:CloneProfile){val opts=arrayOf("إعادة تسمية",if(c.favorite)"إلغاء المفضلة"else"إضافة للمفضلة",if(c.frozen)"إلغاء التجميد"else"تجميد",if(c.hidden)"إظهار"else"إخفاء","تحديث النسخة من التطبيق الأصلي","مسح البيانات","حذف النسخة");showStyled(MaterialAlertDialogBuilder(this).setTitle(c.customName).setItems(opts){_,w->when(w){0->rename(c);1->update(c){it.copy(favorite=!it.favorite)};2->update(c){it.copy(frozen=!it.frozen)};3->update(c){it.copy(hidden=!it.hidden)};4->updateCloneRuntime(c);5->engine.clearData(c.packageName,c.slot).onSuccess{toast("تم مسح بيانات النسخة")}.onFailure{toast("تعذر مسح البيانات: ${it.message?:"خطأ غير معروف"}")};6->confirmDelete(c)}})}
    ...
    private fun confirmDelete(c:CloneProfile){showStyled(MaterialAlertDialogBuilder(this).setTitle("حذف النسخة؟").setMessage("لن يتم حذف التطبيق الأصلي.").setPositiveButton("حذف"){_,_->engine.remove(c.packageName,c.slot);val x=items;x.removeAll{it.id==c.id};store.save(x);render()}.setNegativeButton("إلغاء",null))}
    ...
    private fun settingsDialog(){val options=if(BuildConfig.DEBUG)arrayOf("حالة المحرك: ${engine.name}","إعدادات البطارية","إعدادات الإشعارات","التشخيص","حول التطبيق")else arrayOf("حالة المحرك: ${engine.name}","إعدادات البطارية","إعدادات الإشعارات","حول التطبيق");showStyled(MaterialAlertDialogBuilder(this).setTitle("الإعدادات").setItems(options){_,i->if(BuildConfig.DEBUG){when(i){1->startActivity(Intent(Settings.ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS));2->startActivity(Intent(Settings.ACTION_APP_NOTIFICATION_SETTINGS).putExtra(Settings.EXTRA_APP_PACKAGE,packageName));3->startActivity(Intent(this,DebugActivity::class.java));4->aboutDialog()}}else{when(i){1->startActivity(Intent(Settings.ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS));2->startActivity(Intent(Settings.ACTION_APP_NOTIFICATION_SETTINGS).putExtra(Settings.EXTRA_APP_PACKAGE,packageName));3->aboutDialog()}}})}
