...
object RuntimeProcessAllocator {
    ...
    fun allocate(context: Context, packageName: String, slot: Int, poolSize: Int): Int = synchronized(this) {
        require(poolSize > 0)
        val safeContext = physicalContext(context)
        Runtime3ProcessMetadata.read(safeContext, packageName, slot, poolSize)?.let { persisted ->
            persistPreference(safeContext, packageName, slot, persisted)
            return persisted
        }
        ...
        if (existing in 0 until poolSize) {
            Runtime3ProcessMetadata.write(safeContext, packageName, slot, existing)
            return existing
        }
        ...
        Runtime3ProcessMetadata.write(safeContext, packageName, slot, selected)
    ...
    fun reconcileRunningProcess(context: Context, packageName: String, slot: Int, actualIndex: Int, poolSize: Int): Int = synchronized(this) {
        ...
        val before = lookup(safeContext, packageName, slot, poolSize)
        if (before != actualIndex) {
            ...
            Runtime3ProcessMetadata.write(safeContext, packageName, slot, actualIndex)
    ...
    fun lookup(context: Context?, packageName: String, slot: Int, poolSize: Int): Int? {
        context ?: return null
        val safeContext = physicalContext(context)
        Runtime3ProcessMetadata.read(safeContext, packageName, slot, poolSize)?.let { return it }
        val value = safeContext.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getInt(key(packageName, slot), -1)
        if (value in 0 until poolSize) {
            runCatching { Runtime3ProcessMetadata.write(safeContext, packageName, slot, value) }
            return value
        }
        return null
    }
    ...
    fun release(context: Context, packageName: String, slot: Int) = synchronized(this) {
        val safeContext = physicalContext(context)
        runCatching { RuntimeNotificationBridge.clearClone(safeContext, packageName, slot, removeChannels = true) }
            .onFailure { RuntimeDiagnostics.log("NOTIFY3", "release cleanup failed $packageName/$slot: ${it.javaClass.simpleName}: ${it.message}") }
        Runtime3ProcessMetadata.delete(safeContext, packageName, slot)
        val removed = safeContext.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().remove(key(packageName, slot)).commit()
    ...
    fun snapshot(context: Context): Map<String, Int> = physicalContext(context)
        .getSharedPreferences(PREFS, Context.MODE_PRIVATE).all.mapNotNull { (name, value) ->
    ...
    private fun physicalContext(context: Context): Context {
        ...
        return app ?: MultiApplication.current ?: context
    }
