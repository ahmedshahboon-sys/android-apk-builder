...
class DebugActivity : Activity() {
    companion object {
        private const val COPY_PART_TARGET = 7000
    }
    ...
    private fun auditAll() {
        ...
        Thread {
            val reports = clones.map { clone ->
                clone to runCatching {
                    RuntimeCompatibilityAudit.run(this@DebugActivity, app.engine, clone.packageName, clone.slot)
                }
            }
            val structuralIssues = reports.flatMap { (clone, result) ->
                result.fold(
                    ...
                    onFailure = { error ->
                        listOf(Triple(clone, "AUDIT-CRASH", "${error.javaClass.name}: ${error.message.orEmpty().take(360)}"))
                    }
                )
            }
            ...
            val full = buildString {
                ...
                reports.forEach { (clone, result) ->
                    appendLine(result.fold(
                        onSuccess = { it.render() },
                        onFailure = { "✕ ${clone.customName} (${clone.packageName} #${clone.slot + 1})\n${it.stackTraceToString()}" }
                    ))
                    appendLine()
                }
                appendLine("=== LIVE RUNTIME DIAGNOSTICS ===")
            ...
            lastFullAudit = full
            copyParts = buildTransferParts(full)
            runOnUiThread {
                if (!isFinishing) {
                    fullMode = false
                    output.text = compact
    ...
    private fun prepareCopyParts() {
        val source = if (lastFullAudit.isBlank()) RuntimeDiagnostics.snapshot() else lastFullAudit
        copyParts = buildTransferParts(source)
        renderPartButtons()
        Toast.makeText(this, "تم تجهيز ${copyParts.size} جزء للنسخ", Toast.LENGTH_SHORT).show()
    }
    ...
    private fun buildTransferParts(report: String): List<String> {
        ...
        return rawParts.mapIndexed { index, body ->
            val part = index + 1
            val total = rawParts.size
            buildString {
                appendLine("=== SHAHBOUN REPORT • PART $part/$total ===")
                appendLine("Session: ${RuntimeDiagnostics.currentSessionGroup()}")
                appendLine()
                append(body.trimEnd())
                appendLine(); appendLine()
                if (part < total) {
                    append("ممنوع بدء التعديل أو الإصلاح الآن. هذا الجزء $part من $total فقط. رد فقط: تمام، أنا في الانتظار لباقي التقارير.")
                } else {
                    append("هذا هو الجزء الأخير ($part/$total). ما هي المشاكل التي استنتجتها من جميع الأجزاء؟ حلّل فقط ولا تبدأ الإصلاح حتى أقول لك: ابدأ في الإصلاح.")
                }
            }
        }
    }
    ...
    private fun splitOnLineBoundaries(text: String, target: Int): List<String> {
        ...
        text.lineSequence().forEach { line ->
            ...
            if (current.isNotEmpty() && current.length + addition > target) {
            ...
            if (addition > target) {
                ...
                while (remaining.length > target) {
                    ...
                    parts += remaining.substring(0, cut).trimEnd()
    ...
    private fun safeSentenceCut(value: String, limit: Int): Int {
        ...
        for (i in minOf(limit, value.length - 1) downTo floor) {
            if (value[i] == ' ' || value[i] == '.' || value[i] == ':' || value[i] == ';' || value[i] == '،') return i + 1
        }
        return minOf(limit, value.length)
    }

    private fun clearPartButtons() {
        copyParts = emptyList()
        if (::partsBox.isInitialized) partsBox.removeAllViews()
        if (::partsTitle.isInitialized) partsTitle.text = "أجزاء التقرير: جاري التجهيز…"
    }

    private fun renderPartButtons() {
        partsBox.removeAllViews()
        if (copyParts.isEmpty()) {
            partsTitle.text = "أجزاء التقرير: لا توجد أجزاء جاهزة"
            return
        }
        partsTitle.text = "أجزاء التقرير: ${copyParts.size} • انسخهم بالترتيب"
        copyParts.forEachIndexed { index, text ->
            val label = "نسخ ${index + 1}/${copyParts.size}"
            val b = button(label) { copyPart(index, text) }
            partsBox.addView(b, LinearLayout.LayoutParams(dp(112), dp(42)).apply {
                setMargins(dp(4), 0, dp(4), 0)
            })
        }
    }

    private fun copyPart(index: Int, text: String) {
        val clipboard = getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
        clipboard.setPrimaryClip(ClipData.newPlainText("Shahboun Report ${index + 1}/${copyParts.size}", text))
        Toast.makeText(this, "تم نسخ الجزء ${index + 1} من ${copyParts.size}", Toast.LENGTH_SHORT).show()
    }

    private fun shareCurrent() {
        val text = if (lastFullAudit.isBlank()) RuntimeDiagnostics.snapshot() else lastFullAudit
        val intent = Intent(Intent.ACTION_SEND).apply {
            type = "text/plain"
            putExtra(Intent.EXTRA_SUBJECT, "Shahboun Deep Diagnostics")
