...
public class MainActivityV15 extends MainActivityV14 {
    ...
    @Override public void onCreate(Bundle b){
        super.onCreate(b);
        addDiagnosticChip();
        log("APP","START","Shahboun Router E5785 v1.5.0");
    ...
    private void addDiagnosticChip(){
        diagChip=btn("التشخيص • 0",v->showDiagnostic());
        diagChip.setTextSize(11);diagChip.setPadding(dp(10),0,dp(10),0);
        try{root.addView(diagChip,1,new LinearLayout.LayoutParams(-1,dp(42)));}catch(Exception e){root.addView(diagChip,new LinearLayout.LayoutParams(-1,dp(42)));}
    }

    private String now(){return new SimpleDateFormat("HH:mm:ss.SSS",Locale.US).format(new Date());}
    private void log(String kind,String name,String details){
        if(!diagEnabled)return;
        synchronized(logLock){seq++;diag.append('#').append(String.format(Locale.US,"%03d",seq)).append("  ").append(now()).append("  [").append(kind).append("] ").append(name).append('\n').append(details==null?"":redact(details)).append("\n\n");}
        runOnUiThread(()->{if(diagChip!=null)diagChip.setText("التشخيص • "+seq);});
    }
    private String redact(String s){
        if(s==null)return "";
        s=s.replaceAll("(?is)<Password>.*?</Password>","<Password>***</Password>");
        s=s.replaceAll("(?i)(__RequestVerificationToken[:=]\\s*)[^\\s]+","$1***");
        s=s.replaceAll("(?i)(SessionID=)[^;\\s]+","$1***");
    ...
    private void showDiagnostic(){
        String body=snapshotLog();if(body.isEmpty())body="لا توجد عمليات مسجلة بعد.";
        final String out=body;
        styledMessage("سجل التشخيص",out,"نسخ السجل",()->copyReport(out));
    }
    private void copyReport(String s){
        ClipboardManager cm=(ClipboardManager)getSystemService(CLIPBOARD_SERVICE);cm.setPrimaryClip(ClipData.newPlainText("Shahboun E5785 diagnostics",s));toast("تم نسخ سجل التشخيص");
    }
    private void copyFullReport(){
        StringBuilder s=new StringBuilder();s.append("SHAHBOUN ROUTER E5785 DIAGNOSTIC REPORT\nVersion: 1.5.0\n");
    ...
    @Override String getSync(String path){
        long t=System.currentTimeMillis();String r=super.getSync(path);long ms=System.currentTimeMillis()-t;
        log("GET",path,"duration_ms="+ms+"\nstate="+stateOf(r)+"\nhuawei_code="+pick(tagAny(r,"code"),"none")+"\nresponse=\n"+r);return r;
    }
    @Override String postSync(String path,String xml){
        long t=System.currentTimeMillis();log("POST-REQUEST",path,"request=\n"+xml);String r=super.postSync(path,xml);long ms=System.currentTimeMillis()-t;
        log("POST-RESPONSE",path,"duration_ms="+ms+"\nstate="+stateOf(r)+"\nhuawei_code="+pick(tagAny(r,"code"),"none")+"\nresponse=\n"+r);return r;
    }
    ...
    @Override void apiExplorer(){
        LinearLayout l=form();
        TextView copy=btn("نسخ تقرير كامل",v->copyFullReport());
        TextView clear=btn("مسح السجل",v->clearLog());
        TextView toggle=btn(diagEnabled?"إيقاف التسجيل":"تشغيل التسجيل",v->{diagEnabled=!diagEnabled;((TextView)v).setText(diagEnabled?"إيقاف التسجيل":"تشغيل التسجيل");toast(diagEnabled?"تم تشغيل التسجيل":"تم إيقاف التسجيل");});
        EditText e=input("API path","/api/device/information");
        l.addView(copy,new LinearLayout.LayoutParams(-1,dp(50)));l.addView(clear,new LinearLayout.LayoutParams(-1,dp(50)));l.addView(toggle,new LinearLayout.LayoutParams(-1,dp(50)));l.addView(e,new LinearLayout.LayoutParams(-1,dp(58)));
        styledMessage("أدوات التشخيص","التسجيل يبدأ تلقائيًا عند فتح التطبيق. جرّب الأزرار التي فيها مشاكل، ثم اضغط «نسخ تقرير كامل» وأرسل النص.\n\nكلمات المرور وSessionID وVerification Token يتم إخفاؤها من التقرير.","فتح السجل",this::showDiagnostic);
    }
}
